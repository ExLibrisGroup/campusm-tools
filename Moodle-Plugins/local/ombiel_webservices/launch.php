...
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.
 ...
 /**
  * Launch script, launch the app using ExLibris AEK.
  *
  * @package    ombiel_webservices
  * Copied from package local_mobile which is:
  * @copyright  2014 Juan Leyva <juan@moodle.com> 
  * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
  *
  * The changes from launch.php in local_mobile are:
  * 1. The final redirect which redirects to the AEK page given in the settings and *only* that page.
  * 2. The optional urlscheme parameter has been removed and replaced with the required parameter apptype 
  *     which can be "web" or "native". This lets the final redirect know how to format the url of the AEK page. 
  * 3. is_ios function removed as we are not using custom url schemes
  * 4. The check to see if the plugin is enabled changed to check this plugin no local_mobile
  */  
...
$apptype           = required_param('apptype', PARAM_ALPHA);

if (!in_array($apptype, array('web','native')) ) {  
    print_error('missingparam', '', '', 'apptype');
}
...
$aekscreen = get_config('local_ombiel_webservices', 'campusmaekscreen');
if (empty($aekscreen)) {
    throw new moodle_exception('pluginnotenabledorconfigured', 'local_ombiel_webservices');
}
...
$hostname = get_config('local_ombiel_webservices', 'campusmhostname');
$aekscreen = get_config('local_ombiel_webservices', 'campusmaekscreen');

if ($apptype == 'web') {
    $location = "https://{$hostname}/aek/c/{$aekscreen}?token=$apptoken";
} else {
    $location = "https://{$hostname}/c/{$aekscreen}?token=$apptoken";
}
